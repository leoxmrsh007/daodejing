...
class VirtualCommentator:
    ...
    def get_available_commentators(self, chapter_id: int) -> List[Dict]:
        ...
        chapter = next(
            (c for c in self.data["chapters"] if c["chapter"] == chapter_id), None
        )
    ...
    def chat_with_commentator(
        ...
        response = self._generate_response(
            persona, chapter, commentary_text, user_message
        )
    ...
    def _classify_question(self, question: str) -> str:
        ...
        # 比较类问题：包含区别、差异、不同、比较、对比等关键词
        comparison_keywords = ["区别", "差异", "不同", "比较", "对比", "哪个好", "优劣"]
        if any(keyword in question_lower for keyword in comparison_keywords):
            return "comparison"

        # 含义类问题：包含意思、含义、解释、理解、什么是、为何等关键词
        meaning_keywords = [
            "意思",
            "含义",
            "解释",
    ...
    def _generate_response(
        ...
        chapter: Dict,  # noqa: U100
        commentary: str,  # noqa: U100
        question: str,  # noqa: U100
    ) -> str:
        """回应关于含义的问题"""
        # 返回提示，由LLM生成最终答案
        return f"[AI需要基于{persona.name}的注释风格回答: {question}]"

    def _respond_comparison(
        self,
        persona: CommentaryPersona,
        chapter: Dict,  # noqa: U100
        commentary: str,  # noqa: U100
    ...
    def initiate_debate(
        self, chapter_id: int, topic: str, commentators: List[str]
    ) -> Dict:
        """发起注释家辩论"""
        debate = {
            "chapter": chapter_id,
    ...
    def _generate_opening_statement(
        ...
        return templates.get(
            persona.name.lower(), f'{persona.name}关于"{topic}"的观点。'
        )
...
class SocraticDialogue:
    ...
    def __init__(self) -> None:
        ...
        self.concepts_covered: List = []

    def start_dialogue(
        self, chapter_id: int, user_knowledge_level: str = "beginner"
    ) -> Dict:
        """开始苏格拉底式对话"""
        ...
        return {
            ...
            "opening_question": opening_questions.get(
                chapter_id, f"请谈谈你对第{chapter_id}章的理解。"
            ),
    ...
    def follow_up_question(self, chapter_id: int, user_answer: str) -> str:
        """根据用户回答生成追问"""
        # 检查对话状态但不使用
        self.dialogue_state.get(chapter_id, {})
...
def generate_commentary_response(
    ...
    chapter_id: int,
    question: str,  # noqa: U100
    conversation_context: Optional[List[Dict]] = None,  # noqa: U100
) -> Dict:
    """生成注释家回应（API入口）"""
    ...
    persona = COMMENTATOR_PERSONAS.get(commentator_id)
    if not persona:
        return {"error": "未找到该注释家", "commentator_id": commentator_id}

    # 获取章节内容
    chapter = next(
        (c for c in commentator.data["chapters"] if c["chapter"] == chapter_id), None
    )

    if not chapter:
        return {"error": "未找到该章节", "chapter_id": chapter_id}
...
def start_commentator_debate(
    chapter_id: int, topic: str, commentators: List[str]
) -> Dict:
    """发起注释家辩论"""
    commentator = VirtualCommentator("data/daodejing.json")
