...
class SemanticArchaeology:
    ...
    def load_data(self) -> Dict[str, Any]:
        ...
        try:
            ...
            assert self.data is not None  # for type checker
            return self.data
        except FileNotFoundError:
            self.data = {"title": "道德经", "chapters": []}
            return self.data

    def analyze_text_evolution(self, chapter_id: int) -> Optional[Dict]:
        """分析文本演变"""
        ...
        # 通行本
        evolution["versions"].append(
            {"version": "received", "text": received_text, "diff_note": "原文"}
        )
        ...
        evolution["semantic_shifts"] = self._analyze_semantic_shifts(
            chapter.get("guodian_diff", ""), chapter.get("postsilk_diff", "")
        )
    ...
    def _detect_variants(
        ...
        if postsilk and received:
            # 简单的字符级比较
            diff = list(
                difflib.unified_diff(received.split(), postsilk.split(), lineterm="")
            )
            if diff:
                variants.append(
                    {
    ...
    def _analyze_semantic_shifts(
        self, guodian_diff: str, postsilk_diff: str
    ) -> List[Dict]:
        """分析语义变迁"""
        shifts = []
    ...
    def compare_interpretation_history(
        self, chapter_id: int, concept: str
    ) -> Optional[Dict]:
        """比较历代对同一概念的阐释历史"""
        assert self.data is not None
        chapter = next(
...
# 便捷函数
def get_chapter_archaeology(chapter_id: int) -> Dict[str, Optional[Dict]]:
    """获取章节语义考古分析"""
...
def get_concept_interpretation_history(chapter_id: int, concept: str) -> Optional[Dict]:
    """获取概念阐释历史"""
    archaeology = SemanticArchaeology("data/daodejing.json")
    archaeology.load_data()
