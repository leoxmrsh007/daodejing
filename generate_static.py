...
<head>
    ...
    <script>
    (function(){{var t=localStorage.getItem('daodejing_theme');if(t){{document.documentElement.setAttribute('data-theme',t)}}else{{var d=window.matchMedia('(prefers-color-scheme:dark)').matches;document.documentElement.setAttribute('data-theme',d?'dark':'light')}}}})();
    </script>
    <title>{title}</title>
    <meta name="description" content="古籍经典多版本对照平台 - 道德经 · 庄子 · 周易 · 黄帝内经 · 金刚经 · 四书 · 传习录 · 六祖坛经 · 唯识三十颂">
    ...
    <nav class="navbar navbar-expand-lg navbar-dark bg-primary sticky-top">
        <div class="container-fluid">
            ...
            <div class="ms-auto d-flex align-items-center gap-2">
                ...
                <!-- 设置按钮 -->
                <button class="btn btn-sm btn-outline-light" id="settingsToggle" title="阅读设置">
                    <span>⚙️</span>
                </button>
...
def generate_main_index_page(classics, default_classic):
    ...
    for classic in classics:
        classic_id = classic["id"]
        short_name = classic["short_name"]
        icon = classic.get("icon", "📚")
        color = classic.get("color", "#d4a574")
        description = classic.get("description", "")
        author = classic.get("author", "")
        era = classic.get("era", "")
        chapters = classic.get("chapters", 0)

        classics_html += f"""
        <div class="col-12 col-md-6 col-lg-4 mb-4">
            <a href="./{classic_id}/index.html" class="classic-card">
                <div class="classic-icon" style="color: {color}">{icon}</div>
                <div class="classic-info">
                    <h3 class="classic-name">{short_name}</h3>
                    <p class="classic-author">{author} · {era}</p>
                    <p class="classic-desc">{description}</p>
                    <span class="classic-chapters">{chapters} 章</span>
                </div>
            </a>
        </div>
        """
...
<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title>古籍经典多版本对照平台</title>
    <meta name="description" content="道德经、庄子、周易等经典多版本对照研究平台">
    <link rel="icon" href="data:image/svg+xml,<svg xmlns=%22http://www.w3.org/2000/svg%22 viewBox=%220 0 100 100%22><text y=%22.9em%22 font-size=%2290%22>📚</text></svg>">
    <link href="https://cdn.jsdelivr.net/npm/bootstrap@5.3.0/dist/css/bootstrap.min.css" rel="stylesheet">
    <style>
        body {{
...
{classics_html}
        ...
        <footer class="footer">
            <small>古籍经典多版本对照平台 | 道德经 · 庄子 · 周易 · 六祖坛经 · 金刚经 · 唯识三十颂 · 传习录 · 四书 · 黄帝内经</small>
        </footer>
    </div>
</body>
</html>
